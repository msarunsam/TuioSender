...
int _max_area=50000; //200000		// default: 14 400; 	good: 20 000 for fingertips		[1; infinity]
	...
	if (!min_rect.empty() || !min_r.empty()) {
		if (SYSTEM_INPUT == 1) {
			if (min_r.size() > 0) {
				...
				for (int id = 0; id < int(min_rect.size()); ++id) {
					min_box = fitEllipse( min_r[id] );
					float scaled_touch_x = min_box.center.x / 1392;
					float scaled_touch_y = min_box.center.y / 1044;

					sender.addCursor(scaled_touch_x, scaled_touch_y);

					std::cout << "Touchpoint " << id << ": " << "[" << scaled_touch_x << ", " << scaled_touch_y << "]" << std::endl;
				}
			...
			for (int id = 0; id < int(min_rect.size()); ++id) {
				min_box = fitEllipse( min_r[id] );			// draw an ellipse for each touchpoint
